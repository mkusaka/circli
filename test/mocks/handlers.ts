...
export const mockJob = {
  ...
  project: { slug: "gh/test/repo", name: "repo", external_url: "https://github.com/test/repo" },
  job_number: 456,
  name: "test-job",
  status: "success",
  type: "build",
...
export const handlers = [
  ...
  http.get(`${BASE_URL}/project/:projectSlug/:jobNumber/artifacts`, () => {
    return HttpResponse.json({
      items: [{ path: "artifact.txt", url: "https://example.com/artifact.txt", node_index: 0 }],
      next_page_token: null,
    });
  }),
  ...
  http.get(`${BASE_URL}/project/:projectSlug/:jobNumber/tests`, () => {
    return HttpResponse.json({
      items: [{ name: "test1", classname: "TestClass", result: "success", message: "", run_time: 1.5, source: "jest" }],
      next_page_token: null,
    });
  }),
  ...
  http.get(`${BASE_URL}/context/:contextId/environment-variable`, () => {
    return HttpResponse.json({
      items: [{ variable: "TEST_VAR", context_id: "test-context-id", created_at: "2024-01-01T00:00:00Z", updated_at: "2024-01-01T00:00:00Z" }],
      next_page_token: null,
    });
  }),

  http.put(`${BASE_URL}/context/:contextId/environment-variable/:envVarName`, () => {
    return HttpResponse.json({ variable: "TEST_VAR", context_id: "test-context-id", created_at: "2024-01-01T00:00:00Z", updated_at: "2024-01-01T00:00:00Z" });
  }),
  ...
  http.get(`${BASE_URL}/context/:contextId/restrictions`, () => {
    return HttpResponse.json({
      items: [{ id: "restriction-id", name: "test-restriction", restriction_type: "project", restriction_value: "gh/test/repo" }],
      next_page_token: null,
    });
  }),

  http.post(`${BASE_URL}/context/:contextId/restrictions`, () => {
    return HttpResponse.json({ id: "restriction-id", name: "test-restriction", restriction_type: "project", restriction_value: "gh/test/repo" });
  }),
  ...
  http.get(`${BASE_URL}/project/:projectSlug/checkout-key`, () => {
    return HttpResponse.json({
      items: [{ "public-key": "ssh-rsa AAA...", type: "deploy-key", fingerprint: "aa:bb:cc", preferred: true, "created-at": "2024-01-01T00:00:00Z" }],
      next_page_token: null,
    });
  }),

  http.post(`${BASE_URL}/project/:projectSlug/checkout-key`, () => {
    return HttpResponse.json({ "public-key": "ssh-rsa AAA...", type: "deploy-key", fingerprint: "aa:bb:cc", preferred: true, "created-at": "2024-01-01T00:00:00Z" });
  }),
  ...
  http.get(`${BASE_URL}/insights/:projectSlug/workflows/:workflowName`, () => {
    return HttpResponse.json({
      items: [{ id: "run-id", status: "success", duration: 300, created_at: "2024-01-01T00:00:00Z", stopped_at: "2024-01-01T00:05:00Z", credits_used: 10, branch: "main" }],
      next_page_token: null,
    });
  }),
  ...
  http.get(`${BASE_URL}/insights/:projectSlug/workflows/:workflowName/summary`, () => {
    return HttpResponse.json({
      metrics: { success_rate: 0.95, total_runs: 100, failed_runs: 5, successful_runs: 95, throughput: 10, mttr: 300, duration_metrics: { min: 60, mean: 300, median: 280, max: 600, p95: 500, standard_deviation: 50, total_duration: 30000 } },
      trends: {},
    });
  }),
  ...
  http.get(`${BASE_URL}/insights/:projectSlug/workflows/:workflowName/test-metrics`, () => {
    return HttpResponse.json({
      ...
      slowest_tests: [{ test_name: "slow-test", file: "slow.js", p95_duration: 10.5, flaky: false }],
      slowest_tests_extra: 0,
      test_runs: [{ success_rate: 0.95, pipeline_number: 123, test_counts: { error: 0, failure: 5, skipped: 2, success: 93, total: 100 } }],
      total_test_runs: 100,
    });
  }),
  ...
  http.get(`${BASE_URL}/insights/time-series/:projectSlug/workflows/:workflowName/jobs`, () => {
    return HttpResponse.json({
      items: [{ name: "test-job", min_started_at: "2024-01-01T00:00:00Z", max_ended_at: "2024-01-01T00:30:00Z", timestamp: "2024-01-01T00:00:00Z", metrics: { success_rate: 0.95, total_runs: 50, failed_runs: 3, successful_runs: 47, duration_metrics: { min: 60, mean: 300, median: 280, max: 600, p95: 500, total_duration: 15000 } } }],
      next_page_token: null,
    });
  }),
  ...
  http.get(`${BASE_URL}/insights/pages/:projectSlug/summary`, () => {
    return HttpResponse.json({
      org_id: "org-id",
      project_id: "project-id",
      project_data: { metrics: { success_rate: 0.95, total_runs: 100, total_credits_used: 1000 }, trends: {}, workflows: {} },
    });
  }),

  http.get(`${BASE_URL}/insights/:orgSlug/summary`, () => {
    return HttpResponse.json({
      org_data: { metrics: { success_rate: 0.95, total_runs: 1000, total_credits_used: 10000 }, trends: {} },
      org_project_data: [],
      all_projects: [],
    });
  }),
  ...
  http.get(`${BASE_URL}/owner/:ownerID/context/:context/policy-bundle`, () => {
    return HttpResponse.json({ policies: { "policy1.rego": "package policy1" } });
  }),
  ...
  http.get(`${BASE_URL}/me/collaborations`, () => {
    return HttpResponse.json([
      { id: "org-id", name: "test-org", slug: "gh/test-org", vcs_type: "github" },
    ]);
  }),
